...
class App extends Component {
  ...
  componentDidMount() {
    ...
    .then(results => {
      ...
      const attendanceRewarder = results.web3.eth.contract(AttendanceRewarder.abi);
      
      //const instance = attendanceRewarder.at('0xeED1D1566b90dF23770648ba9ca1B754D3fFF7BF');
      const instance = eth.contract(AttendanceRewarder.abi).at('0xeED1D1566b90dF23770648ba9ca1B754D3fFF7BF');
      this.setState({
    ...
    */
    ...
    setInterval(() => {
      ...
      if(this.state.web3.eth.accounts[0] !== account) {
        ...
        let hasContributed = this.state.attendeeList.indexOf(this.state.loggedInAddress) > -1;
  ...
  addTxnId(txId){
    let hasTxId = this.state.txnHashes.indexOf(txId) > -1;
    if(hasTxId === false){
      let txIds = [...this.state.txnHashes, txId];
      this.setState({txnHases: txIds});
    }
    return hasTxId;
  }
    ...
    this.state.web3.eth.getAccounts((err, accounts) => {
      if (err !== null){
        ...
        this.setState({isLoggedIn: true});
         contractInstance.getEventId().then((result) => {
          // Get the value from the contract to prove it worked.
          this.setState({ eventId: result[0].toString() })
          return contractInstance.getAttendeeList().then()
        }).then((result) => {
          // Update state with the result.
          this.setState({ attendeeList: result })
          return contractInstance.getCurrentEventValue().then
        }).then((result) => {
          // Get the value from the contract to prove it worked.
          this.setState({ currentEventValue: result.toString() });
          let hasContributed = this.state.attendeeList.indexOf(this.state.loggedInAddress) > - 1;
          this.setState({hasContributed: hasContributed});
          this.lookupUserWinnings();
        }).catch((err) => {
  ...
  getAttendeeData(){
    let contractInstance = this.state.contractInstance;
    contractInstance.getAttendeeData({}).then((result) => {
  ...
  lookupUserWinnings(){
    ...
    this.state.web3.eth.getAccounts((error, accounts) => {
      contractInstance.checkUnclaimedWinnings({from: accounts[0]}).then((result) => {
        this.setState({loggedInUserWinnings: (result.toString() / this.state.ether)})
      }).catch((err) => {
        console.log("Failed with error: " + err);
      });
    })
